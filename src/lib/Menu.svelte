<script lang="ts">
	...
	const categories: string[] = ['All', 'Biryani', 'Kebab'];

	const items: MenuItem[] = [
		{
			name: 'Chicken Biryani',
			cat: 'Biryani',
			img: '/assets/images/Chicken Biryani.webp',
			prices: [
				'Good for 1 + Karak — ₱458',
				'Good for 2 + Karak — ₱698',
				'Good for 3 + Karak — ₱1,158',
				'Family (4–5) + Karak — ₱1,858'
			],
			popular: true
		},
		{
			name: 'Mutton Biryani',
			cat: 'Biryani',
			img: '/assets/images/Mutton Biryani.webp',
			prices: [
				'Good for 1 + Karak — ₱768',
				'Good for 2 + Karak — ₱1,168',
				'Good for 3 + Karak — ₱1,985',
				'Family + Karak — ₱3,078'
			],
			popular: true
		},
		{
			name: 'Beef Biryani',
			cat: 'Biryani',
			img: '/assets/images/Beef Biryani.webp',
			prices: [
				'Good for 1 + Karak — ₱508',
				'Good for 2 + Karak — ₱778',
				'Good for 3 + Karak — ₱1,288',
				'Family + Karak — ₱2,058'
			],
			popular: false
		},
		{
			name: 'Fish Biryani',
			cat: 'Biryani',
			img: '/assets/images/Fish Biryani.webp',
			prices: ['Good for 1 + Karak — ₱588'],
			popular: false
		},
		{
			name: 'Chicken Kebab',
			cat: 'Kebab',
			img: '/assets/images/Chicken Kebab.webp',
			prices: ['Good for 1 + Karak — ₱398'],
			popular: false
		},
		{
			name: 'Beef Kebab',
			cat: 'Kebab',
			img: '/assets/images/Beef Kebab.webp',
			prices: ['Good for 1 + Karak — ₱478'],
			popular: false
		},
		{
			name: 'Shish Tawook',
			cat: 'Kebab',
			img: '/assets/images/Shis Tauwok.webp',
			prices: ['Good for 1 + Karak — ₱568'],
			popular: true
		},
		{
			name: 'Chicken Tikka Kabab',
			cat: 'Kebab',
			img: '/assets/images/Chicken Tikka Kabab.webp',
			prices: ['Good for 1 + Karak — ₱528'],
			popular: false
		},
		{
			name: 'Chicken Tikka Masala',
			img: '/assets/images/Chicken Tikka-Masala.webp',
			prices: ['Good for 1 + Karak — ₱238'],
			popular: false
		}
	];

	const filtered: MenuItem[] = $derived(
		selected === 'All' ? items : items.filter((i) => i.cat === selected)
	);
	...
	<div class="container">
			...
			<div class="filters" role="tablist">
				{#each categories as c}
					<button
						...
						onclick={() => (selected = c)}
						role="tab"
						aria-selected={selected === c}>{c}</button
					>
				{/each}
			</div>
		</div>
		...
		<div class="cta">
			<button class="btn btn-primary" onclick={openPopup}
				>View Full Menu — 10 Pages <ion-icon name="images-outline"></ion-icon></button
			>
	...
	<div class="popup" role="dialog" aria-modal="true">
		...
		<div class="panel">
			<div class="panel-head">
				...
				<button class="close" onclick={closePopup} aria-label="Close menu"
					><ion-icon name="close-outline"></ion-icon></button
				>
			</div>
			<div class="viewer">
				<button class="arrow left" onclick={prevPage} aria-label="Previous"
					><ion-icon name="chevron-back"></ion-icon></button
				>
				<img src={pages[page]} alt="Menu page {page + 1}" />
				<button class="arrow right" onclick={nextPage} aria-label="Next"
					><ion-icon name="chevron-forward"></ion-icon></button
				>
			</div>
